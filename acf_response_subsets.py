...
def prepare_response_activity(population_activity):
    """
    population_activity: n_units x n_trials x n_bins
    Returns activity averaged across trials and centered in time,
    reshaped to match acf() expected input.
    """
    response_activity = population_activity.mean(axis=1)        # n_units x n_bins
    response_activity -= response_activity.mean(axis=1, keepdims=True)
    response_activity = response_activity[np.newaxis, :, :]      # 1 x n_units x n_bins
...
for subset in SUBSETS:
    ...
    for area in AREAS:
        ...
        response_activity = prepare_response_activity(population_activity)
    ...
    output_file = os.path.join(
        OUTPUT_DIR,
        f"adap_{CONDITION_TAG}_{MODEL_TAG}_subset{subset}_response_r_t_gamma_{GAMMA_TAG}.txt"
    )
...
acf_output_file = os.path.join(
    OUTPUT_DIR,
    f"acf_curves_response_{CONDITION_TAG}_{MODEL_TAG}_gamma_{GAMMA_TAG}.npz"
)
np.savez(
    acf_output_file,
    time=TIME_VECTOR,
...
print(f"\nSaved all ACF response curves: {acf_output_file}")
